...
BEGIN
    ...
    DECLARE stu_count INT DEFAULT 1000; -- 高一学生总数
    ...
    WHILE i <= head_count DO
        ...
        -- 插入账号表 (工号T开头，6位补零)
        INSERT INTO account (person_id, password) VALUES (CONCAT('T', LPAD(i, 6, 0)), '123456');
        -- 插入教职工表
        INSERT INTO staff (staff_id, staff_name, gender, position, is_leave) 
        VALUES (CONCAT('T', LPAD(i, 6, 0)), rand_name, rand_gender, '校长', '否');
        -- 绑定角色：校长 R005
        INSERT INTO account_role (account_id, role_id) VALUES (CONCAT('T', LPAD(i, 6, 0)), 'R005');
        SET i = i + 1;
    END WHILE;


    -- ========== 二、生成【2个教务主任】数据 ==========
    WHILE i <= head_count + dean_count DO
        SET rand_gender = IF(RAND()>0.5, '男', '女');
        SET rand_name = get_rand_name(rand_gender);
        INSERT INTO account (person_id, password) VALUES (CONCAT('T', LPAD(i, 6, 0)), '123456');
        INSERT INTO staff (staff_id, staff_name, gender, position, is_leave) 
        VALUES (CONCAT('T', LPAD(i, 6, 0)), rand_name, rand_gender, '教务主任', '否');
        -- 绑定角色：教务主任 R004
        INSERT INTO account_role (account_id, role_id) VALUES (CONCAT('T', LPAD(i, 6, 0)), 'R004');
        SET i = i + 1;
    END WHILE;

    -- ========== 三、生成【40个教师】数据 (前10个为班主任+科任老师，后30个仅科任老师) ==========
    WHILE i <= head_count + dean_count + tea_count DO
        SET rand_gender = IF(RAND()>0.5, '男', '女');
        SET rand_name = get_rand_name(rand_gender);
        -- 插入账号表
        INSERT INTO account (person_id, password) VALUES (CONCAT('T', LPAD(i, 6, 0)), '123456');
        -- 插入教职工表
        INSERT INTO staff (staff_id, staff_name, gender, position, is_leave) 
        VALUES (CONCAT('T', LPAD(i, 6, 0)), rand_name, rand_gender, get_rand_position(), '否');
        -- 前10个教师 绑定【班主任 R002 + 科任老师 R003】双角色（支持兼任，无重复唯一键）
        IF i <= head_count + dean_count + 20 THEN
            INSERT INTO account_role (account_id, role_id) VALUES (CONCAT('T', LPAD(i,6,0)), 'R002');
            INSERT INTO account_role (account_id, role_id) VALUES (CONCAT('T', LPAD(i,6,0)), 'R003');
        ELSE
    ...
    SET i = 1;
    WHILE i <= 20 DO
        SET class_id = CONCAT('C1', LPAD(i, 2, 0)); -- 班级编号 C101-C110 (1代表高一)
        SET class_name = CONCAT('高一', i, '班');
        -- 班主任工号精准对应：校长(1)+主任(2) 之后的第1-10个老师，就是班主任
        SET ht_id = CONCAT('T', LPAD(head_count + dean_count + i, 6, 0)); 
        INSERT INTO class (class_id, class_name, grade, subject_combination, head_teacher_id, current_student_num, create_time)
        VALUES (class_id, class_name, 1, '高一行政班，暂无选科', ht_id, 50, DATE_FORMAT(NOW(), '%Y-%m-%d'));
        SET i = i + 1;
    END WHILE;
