...
CREATE TABLE account(
    person_account VARCHAR(20) NOT NULL COMMENT '账号',
    password VARCHAR(20) NOT NULL COMMENT '密码',
    person_id VARCHAR(20) NOT NULL COMMENT '对应用户',
    PRIMARY KEY (person_account)
)COMMENT '账号密码';
...
CREATE TABLE course (
    ...
    course_type ENUM('主科', '选科') NOT NULL COMMENT '课程类型',
...
CREATE TABLE class (
    ...
    subject_combination INT CHECK (subject_combination BETWEEN 0 and 11) COMMENT '选科组合',
...
CREATE TABLE account_role (
    ...
    FOREIGN KEY (account_id) REFERENCES account(person_account) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE student_course_choose (
    ...
    one_choose_one ENUM('物理', '历史') NOT NULL COMMENT '物理/历史二选一',
...
CREATE TABLE exam (
    ...
    is_city_rank ENUM('是', '否') NOT NULL COMMENT '是否有市排名',
...
CREATE TABLE exam_score (
    ...
    student_id VARCHAR(20) NOT NULL COMMENT '学号',
    course_id VARCHAR(20) NOT NULL COMMENT '课程编号',
    original_score DECIMAL(5,1) NOT NULL COMMENT '原始分数',
    convert_score DECIMAL(5,1) COMMENT '赋分分数(政史地生+市统考才有)',
    city_rank INT COMMENT '单科市排名(仅市统考有)',
    valid_exam_num INT COMMENT '单科有效参考人数(仅市统考有)',
    PRIMARY KEY (id),
